...
When installing {VirtProductName}, you select an update channel and an approval strategy. The update channel determines the versions that {VirtProductName} will be updated to, as well as the update frequency. The approval strategy setting determines if updates can occur automatically or if manual approval is required. Both settings can impact supportability.
...
[NOTE]
====
Each minor version of {VirtProductName} is only supported if you run the corresponding {product-title} version. For example, you must run {VirtProductName} {VirtVersion} on {product-title} {VirtVersion}.
====
