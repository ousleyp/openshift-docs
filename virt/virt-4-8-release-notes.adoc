...
* If you initially deployed {VirtProductName} at version 2.4.z or earlier, upgrading to version 4.8 fails with the following message: `risk of data loss updating hyperconvergeds.hco.kubevirt.io: new CRD removes version v1alpha1 that is listed as a stored version on the existing CRD`. This bug does not affect clusters where {VirtProductName} was initially deployed at version 2.5.0 or later. (link:https://bugzilla.redhat.com/show_bug.cgi?id=1986989[*BZ#1986989*])
+
** As a workaround, remove the `v1alpha1` version and continue the upgrade.
+
. Run a proxy to the Kubernetes API server by running the following command:
+
[source,terminal]
----
$ oc proxy &
----
+
. Remove the `v1alpha1` version from `.status.storedVersions` on the `HyperConverged` custom resource definition (CRD) by running the following command:
+
[source,terminal]
----
...
----
+
. Restart the upgrade process by running the following command:
+
[source,terminal]
----
...
----
+
. Kill the `oc proxy` process. For example:
+
[source,terminal]
----
...
----
+
. Verify that the upgrade continues successfully by xref:../virt/upgrading-virt.adoc#virt-monitoring-upgrade-status_upgrading-virt[monitoring the upgrade status].
...
* When creating a virtual machine after upgrading to OCP 4.8 while running CNV 2.6.5, various errors occur.
+
...
----
+
*** As a workaround, modify the `apiVersion` in the manifest.
+
[source,yaml]
----
...
// fix targeted for 4.8.1
* When connecting to the VNC console by using the {VirtProductName} web console and `virtctl` command, the VNC console always fails to respond.
+
** As a workaround, create the virtual machine from the CLI or upgrade to CNV 4.8.
+
